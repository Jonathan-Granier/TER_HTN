...
implements Serializable {
	...
	/**
	 * The goal tasks declared in the problem
	 */
	private Exp goalTasks;
	...
	throws NullPointerException {
		...
		this.name = name;
		requirements = new LinkedHashSet<RequireKey>();
		objects = new ArrayList<TypedSymbol>();
		initialFacts = new ArrayList<Exp>();
		goal = null;
		constraints = null;
		metric = null;
	}
	...
	/**
	 * Returns the types of a defined objects
	 * 
	 * @param constant The object which we desire the type
	 * @return <code> The object's types</code> if it's defined or <code> null </code> otherwise
	 */
	public final List<Symbol> getObjectType(final Symbol constant) {
		int i = 0;
		while (i < objects.size() && !objects.get(i).getImage().equals(constant.getImage())) {
			i++;
		}
		if (i < objects.size()) {
			return objects.get(i).getTypes();
		} else {
			return null;
		}
	}
	 ...
	 */
	public Exp getGoalTasksConstraints() {
		return constraints;
	}
	 ...
	 * @param constraints the constraints to set.
	 */
	public final void setGoalTasks(final Exp goalTasks) {
	...
	public final TypedSymbol getObject(final Symbol symbol) {
		final int index = objects.indexOf(symbol);
	...
	public void standardize() {
		...
		if (!getRequirements().contains(RequireKey.HTN)) {
			getGoal().renameVariables();
			getGoal().moveNegationInward();
		}
		// Rename the constraints of the problem
		else if (getGoalTasksConstraints() != null) {
			getGoalTasksConstraints().renameVariables();
			getGoalTasksConstraints().moveNegationInward();
		}
	}
	...
	public String toString() {
		...
		str.append(")");
		str.append("\n(:requirements");
		for (final RequireKey r : requirements) {
			str.append(" " + r);
		}
		str.append(")\n");
		if (!objects.isEmpty()) {
			str.append("(:objets ");
			for (final TypedSymbol obj : objects) {
				str.append("\n  " + obj);
			}
			str.append("\n)\n");
		}
		str.append("(:init");
		for (final Exp fact : initialFacts) {
			str.append("\n  " + fact);
		}
		str.append("\n)\n");
		str.append("(:goal ");
