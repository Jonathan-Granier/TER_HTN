...
import pddl4j.parser.lexer.Token;
...
public class Symbol implements Serializable {
	 ...
	 */
	public static final String DEFAULT_TAG_TASK_SYMBOL = "_T";
	...
	/**
	 * Renames the symbol from a specified index. The symbol is renamed if only if this symbol is a
	 * task tag, otherwise nothing is done. After rename operation the variable will have the form
	 * <code>!Tn</code> where <code>n</code> is the specified index.
	 * 
	 * @param index the index of the symbol.
	 * @return the old image of the symbol or null if the symbol was not renamed.
	 * @throws IllegalArgumentException if index is < 0.
	 */
	public final String renameTaskTag(final int index) {
		if (index < 0) throw new IllegalArgumentException("index < 0");
		String image = null;
		if (this.getKind().equals(Symbol.Kind.TASK_TAG)) {
			image = this.getImage();
			this.setImage(Symbol.DEFAULT_TAG_TASK_SYMBOL + index);
		}
		return image;
	}

	/**
	 * Renames the symbol from a specified symbol. The symbol is renamed if only if this symbol is a
	...
	/**
	 * Renames the symbol from a specified symbol. The symbol is renamed if only if this symbol is a
	 * task tag, otherwise nothing is done.
	 * 
	 * @param context the images of the already renamed task tag.
	 * @return the old image of the symbol or null if the symbol was not renamed.
	 * @throws NullPointerException if context == null.
	 */
	public final String renameTaskTag(final Map<String, String> context) {
		if (context == null) throw new NullPointerException("context == null");
		String image = null;
		if (this.getKind().equals(Symbol.Kind.TASK_TAG)) {
			image = this.getImage();
