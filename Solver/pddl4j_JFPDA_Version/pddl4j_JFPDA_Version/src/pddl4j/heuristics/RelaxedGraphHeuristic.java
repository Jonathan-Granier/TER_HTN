...
/**
 * This abstract class implements the basic methods used by all heuristics based on the computation of a relaxed planning graph ignoring negative effects. This
 * implementation is based on the implementation of the fast forward planner proposed by J. Hoffmann. The relaxed planning computed does not compute mutual
 * exclusion. To have more information about this implementation see Hoffmann, J. and Nebel, B. (2001). The FF Planning System: Fast Plan Generation Through
...
extends AbstractHeuristic {
	...
	protected RelaxedGraphHeuristic(final CodedProblem problem) {
		...
		for (final Signature sign : operators.keySet()) {
			...
			for (final BitOp op : ops) {
				...
				for (int ce_index = 0; ce_index < effects.size(); ce_index++) {
					...
					unconditional_operators[uncond_op_index] = eff;
					...
					for (int p = p_pre.nextSetBit(0); p >= 0; p = p_pre.nextSetBit(p + 1)) {
						precond_edges[p].getPositive().set(uncond_op_index);
					}
					for (int p = n_pre.nextSetBit(0); p >= 0; p = n_pre.nextSetBit(p + 1)) {
						precond_edges[p].getNegative().set(uncond_op_index);
					}
					
					// We set the preconditions of the unconditional operator
					preconditions[uncond_op_index] = pre;
					...
					for (int p = p_eff.nextSetBit(0); p >= 0; p = p_eff.nextSetBit(p + 1)) {
						effects_edges[p].getPositive().set(uncond_op_index);
					}
					for (int p = n_eff.nextSetBit(0); p >= 0; p = n_eff.nextSetBit(p + 1)) {
						effects_edges[p].getNegative().set(uncond_op_index);
					}
					...
					precond_cardinality[uncond_op_index] = pre.cardinality();
					
					// We initialize the unconditional effects of the operator
					if (c_effect.getCondition().isEmpty()) {
						final BitExp uncond_eff = unconditional_effects[op_index];
		...
		for (int i = 0; i < nb_uncond_operators; i++) {
			if (preconditions[i].isEmpty()) {
				for (final BitExp p_edge : precond_edges) {
	...
	throws NullPointerException {
		super.setGoal(goal);
		goal_cardinality = goal.cardinality();
	}
	...
	protected final int expandRelaxedPlanningGraph(final BitState state) {
		...
		while (goal_counter != goal_cardinality && (!p_pk.isEmpty() || !n_pk.isEmpty())) {
			...
			for (int p = p_pk.nextSetBit(0); p >= 0; p = p_pk.nextSetBit(p + 1)) {
				...
				final BitVector p_edges = precond_edges[p].getPositive();
				// We mark the positive proposition p as explored
				p_acc.set(p);
			...
			for (int p = n_pk.nextSetBit(0); p >= 0; p = n_pk.nextSetBit(p + 1)) {
				// We get the operator that have this positive proposition as precondition
				final BitVector n_edges = precond_edges[p].getNegative();
	 ...
	 * 
	 * @return <code>true</code> if the goal is reachable after the planning graph expansion; <code>false</code> otherwise.
	 */
	protected final boolean isGoalReachable() {
		return goal_cardinality == goal_counter;
	}
	...
	protected final int getMaxValue() {
		...
		final BitVector n_goal = super.getGoal().getNegative();
		for (int g = p_goal.nextSetBit(0); g >= 0; g = p_goal.nextSetBit(g + 1)) {
			final int gl = p_prop_level[g];
			if (gl > max) {
				max = gl;
			}
		}
		for (int g = n_goal.nextSetBit(0); g >= 0; g = n_goal.nextSetBit(g + 1)) {
			final int gl = n_prop_level[g];
			if (gl > max) {
				max = gl;
			}
		}
	...
	protected final int getRelaxedPlanValue() {
		...
		final BitExp[] goals = new BitExp[level + 1];
		...
		final BitVector n_goal = super.getGoal().getNegative();
		for (int g = p_goal.nextSetBit(0); g >= 0; g = p_goal.nextSetBit(g + 1)) {
			goals[p_prop_level[g]].getPositive().set(g);
		}
		for (int g = n_goal.nextSetBit(0); g >= 0; g = n_goal.nextSetBit(g + 1)) {
			goals[n_prop_level[g]].getNegative().set(g);
		}
		...
		for (int k = level; k > 0; k--) {
			...
			for (int p_g = p_gk.nextSetBit(0); p_g >= 0; p_g = p_gk.nextSetBit(p_g + 1)) {
				...
				final int resolver_index = select(effects_edges[p_g].getPositive(), k);
				if (resolver_index != -1) {
					final BitExp pre = preconditions[resolver_index];
					final BitVector p_pre = pre.getPositive();
					for (int p = p_pre.nextSetBit(0); p >= 0; p = p_pre.nextSetBit(p + 1)) {
						final int p_level = p_prop_level[p];
			...
			for (int n_g = n_gk.nextSetBit(0); n_g >= 0; n_g = n_gk.nextSetBit(n_g + 1)) {
				final int resolver_index = select(effects_edges[n_g].getNegative(), k);
				if (resolver_index != -1) {
					final BitExp pre = preconditions[resolver_index];
					final BitVector p_pre = pre.getPositive();
					for (int p = p_pre.nextSetBit(0); p >= 0; p = p_pre.nextSetBit(p + 1)) {
	...
	/**
	 * Select an effect according to the unconditional operators difficulty heuristic. The question is, which achiever should be choose when no NOOP is
	 * available ? It is certainly a good idea to select an achiever whose preconditions seems to be "easy". From the graph building phase, we can obtain a
	 * simple measure for the operators_difficulty of an action's preconditions as follows:
	 * <ul>
	 * <li>operators_difficulty(o) := SUM_ID(min { i | p is member of the fact layer at time i }) with p in pre(o)</li>
	 * </ul>
	 * The operators_difficulty of each action can be set when it is first inserted into the graph. During plan extraction, facing a fact for which no NOOP is
	 * available, we then simply selected an achieving action with minimal operators_difficulty. This heuristic works well in situation where there are severals
	 * ways to achieve one fact. but some ways need less effort than others.
	 * 
	 * @param resolvers the list of resolver of p.
	 * @param k the level.
	 * @return the easier resolver for the proposition <code>p</code> at level <code>k</code> or <code>null</code> if a NOOP operator is available.
	 */
	private int select(final BitVector resolvers, final int k) {
		...
		for (int r = resolvers.nextSetBit(0); r >= 0; r = resolvers.nextSetBit(r + 1)) {
			if (operators_level[r] < k) {
				final int difficulty = operators_difficulty[r];
