...
final class Instantiation {
	 ...
	 * Instantiates a specified operator.
	 * <p>
	 * The assumption is made that different operator parameters are instantiated with different constants, i.e., the planner never generates actions like
	 * move(a,a) because we consider this as a bad domain representation that should be revised. In fact, in operators with identical constant parameters, all
	 * but one of the constants are superfluous and can be skipped from the representation without loss of information.
	 * </p>
	 * 
	 ...
	 */
	static List<IntMeth> instantiateMethods(final List<IntMeth> methods) {
		final List<IntMeth> instMeth = new ArrayList<>(1000);
		for (final IntMeth meth : methods) {
			Instantiation.expandQuantifiedExpression(meth.getConstraints());
			Instantiation.simplify(meth.getConstraints());
			instMeth.addAll(Instantiation.instantiate(meth));
		}
		return instMeth;
	}
	 ...
	 */
	static List<IntMeth> instantiate(final IntMeth method) {
		final List<IntMeth> instMeths = new ArrayList<>(100);
		Instantiation.instantiate(method, 0, Integer.MAX_VALUE, instMeths);
		return instMeths;
	}
	 ...
	 * Instantiates a specified method.
	 * <p>
	 * The assumption is made that different methods parameters are instantiated with different constants, i.e., the planner never generates methods like
	 * move(a,a) because we consider this as a bad domain representation that should be revised. In fact, in method with identical constant parameters, all but
	 * one of the constants are superfluous and can be skipped from the representation without loss of information.
	 * </p>
	 * 
	 ...
	 * @see pddl4j.preprocessing.IntOp#instantiateOperators(int, int)
	 */
	private static void instantiate(final IntMeth meth, final int index, final int bound, final List<IntMeth> methods) {
		if (bound == methods.size()) {
			return;
		}
		
		final int arity = meth.getArity();
		final int undeclaredLength = meth.getUndeclaredParameters().length;
		if (index == arity + undeclaredLength) {
			final List<IntExp> constraints = meth.getConstraints().getChildren();
			final Iterator<IntExp> iter = constraints.iterator();
			boolean rejected = false;
			while (iter.hasNext() && !rejected) {
				final IntExp constraint = iter.next();
				if (!constraint.getConnective().equals(Connective.TASK)) {
					final IntExp con;
					final Connective connect = constraint.getConnective();
					if (connect.equals(Connective.ATOM) || connect.equals(Connective.TRUE) || connect.equals(Connective.FALSE)) {
						con = constraint;
					} else {
						con = constraint.getChildren().get(0);
					}
					if (!con.getConnective().equals(Connective.FALSE)) {
						Instantiation.simplify(con);
						if (con.getConnective().equals(Connective.FALSE)) {
							rejected = true;
						}
					} else {
			...
			if (!rejected) {
				methods.add(meth);
				//System.out.println(" ++ Method added: " + meth.getName());
			} else {
				//System.err.println(" -- Method rejected: " + meth.getName());
			}
		} else {
			int parameterType = -1;
			//Index is the index of a declared Parameter
			if (index < arity) {
				parameterType = meth.getTypeOfParameters(index);
				//Index is the index of an undeclared Parameter
			} else {
				parameterType = meth.getTypeOfUndeclaredParameters(index - arity);
			}
			final Set<Integer> values = Preprocessing.tableOfDomains.get(parameterType);
			for (final Integer value : values) {
				if (!meth.isAlreadyInstantiatedWith(value)) {
					long localNow = System.currentTimeMillis();
					
					final int varIndex = -index - 1;
					final IntExp expansion = new IntExp(meth.getExpansion());
					Instantiation.substitute(expansion, varIndex, value);
					final IntExp constraints = new IntExp(meth.getConstraints());
					Instantiation.substitute(constraints, varIndex, value);
					final IntMeth copy = new IntMeth(meth.getName(), arity, undeclaredLength);
					copy.setExpansion(expansion);
					copy.setConstraints(constraints);
					for (int i = 0; i < arity; i++) {
						copy.setTypeOfParameter(i, meth.getTypeOfParameters(i));
					}
					for (int i = 0; i < undeclaredLength; i++) {
						copy.setTypeOfUndeclaredParameter(i, meth.getTypeOfUndeclaredParameters(i));
					}
					Preprocessing.inst1 += System.currentTimeMillis() - localNow;
					...
					if (index < arity) {
						...
						for (int i = 0; i < index; i++) {
							copy.setValueOfParameter(i, meth.getValueOfParameter(i));
						}
						copy.setValueOfParameter(index, value);
						//Index is the index of an undeclared Parameter
					} else {
						...
						for (int i = 0; i < index - arity; i++) {
							copy.setValueOfUndeclaredParameter(i, meth.getValueOfUndeclaredParameter(i));
						}
						copy.setValueOfUndeclaredParameter(index - arity, value);
					}
					Preprocessing.inst2 += System.currentTimeMillis() - localNow;
		...
		switch (exp.getConnective()) {
			case AND:
				...
				while (i.hasNext() && exp.getConnective().equals(Connective.AND)) {
					...
					if ((ei.getConnective().equals(Connective.FORALL) || ei.getConnective().equals(Connective.EXISTS)) && Preprocessing.tableOfDomains.get(ei.getType()).isEmpty()) {
						i.remove();
						continue;
					}
			...
			case FORALL:
				...
				while (it.hasNext() && exp.getConnective().equals(Connective.AND)) {
					...
					final IntExp copy = new IntExp(qExp);
		...
		switch (exp.getConnective()) {
			...
			case LAST:
				for (final IntExp e : exp.getChildren()) {
					...
					Instantiation.substitute(e, var, cons);
				}
				break;
			case FN_HEAD:
				args = exp.getArguments();
	 ...
	 * <ul>
	 * <li>1. If the expression is a positive inertia and the number of unifying ground instances of the specified expression that are contained in the initial
	 * state is equal to 0 then the expression is simplified to FALSE.</li>
	 * <li>2. If the expression is a negative inertia and then the number of all possible type-consistent ground instances of the specified expression then the
	 * expression is simplified to TRUE.
	 * </ul>
	 * 
