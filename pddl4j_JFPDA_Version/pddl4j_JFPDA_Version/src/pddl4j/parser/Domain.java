...
public class Domain implements Serializable {
	 ...
	 */
	private static final long serialVersionUID = 8404061519943429568L;
	...
	/**
	 * Returns the types of a defined constant
	 * @param constant The constant which we desire the type
	 * @return <code> The constant's types</code> if it's defined or <code> null </code> otherwise
	 */
	public final List<Symbol> getConstantType(Symbol constant){
		int i = 0;
		while(i < constants.size() && !constants.get(i).getImage().equals(constant.getImage())){
			i++;
		}
		if(i < constants.size())
				return constants.get(i).getTypes();
		else
			return null;
	}
	...
	/**
	 * Get the operators that correspond to the passed name and parameters types
	 * 
	 * @param name The name of the searched operator
	 * @param parameters The parameters of the searched operator
	 * @return The list of corresponding operators
	 */
	public final List<Op> getOperator(Symbol name,  List<TypedSymbol> parameters) {
		List<Op> operators = new ArrayList<Op>();
		boolean sameOp;
		for (Op operator : this.getOperators()){
			sameOp=false;
			if( name.equals(operator.getName()) && parameters.size() == operator.getParameters().size()){
				sameOp=true;
				int i=0;
				while(sameOp && i<parameters.size()){
					if(!this.isSubType(parameters.get(i), operator.getParameters().get(i)))
						sameOp = false;
					i++;
				}
	...
	/**
	 * The getter for the methods list
	 * 
	...
	/**
	 * Get the methods that correspond to the passed name and parameters types.
	 * 
	 * @param name The name of the searched method.
	 * @param parameters The parameters of the searched method.
	 * @return The list of corresponding methods.
	 */
	public final List<Method> getMethod(Symbol name,  List<TypedSymbol> parameters) {
		List<Method> methods = new ArrayList<Method>();
		boolean sameMethod;
		for (Method method : this.getMethods()){
			sameMethod=false;
			if( name.equals(method.getName()) && parameters.size() == method.getParameters().size()){
				sameMethod=true;
				int i=0;
				while(sameMethod && i<parameters.size()){
					if(!this.isSubType(parameters.get(i), method.getParameters().get(i)))
						sameMethod = false;
					i++;
				}
	...
	public boolean isSubType(TypedSymbol s1, TypedSymbol s2) {
		...
		while (i.hasNext() && !isSubType) {
			...
			// If the Type of the parameter is not defined return false
	...
	public void standardize() {
		...
		if (this.getRequirements().contains(RequireKey.HTN))
