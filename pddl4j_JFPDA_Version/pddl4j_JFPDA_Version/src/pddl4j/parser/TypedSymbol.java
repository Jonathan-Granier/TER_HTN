...
public final class TypedSymbol extends Symbol {
	...
	/**
	 * Creates a new typed symbol from a specified symbol. This symbol is by
	 * default of type object. If a typed symbol is created with the specified
	 * symbol <code>Parser.OBJECT</code> or <code>Parser.NUMBER</code>, the typed
	 * list is creates with an empty list of super types.
	 * 
	 * @param symbol the symbol.
	...
	public TypedSymbol(final Symbol symbol) throws NullPointerException {
		super(symbol.getKind(), symbol.getImage(), symbol.getBeginLine(), symbol.getBeginColumn(), symbol.getEndLine(), symbol
				.getEndColumn());
		this.types = new ArrayList<Symbol>();
	...
	/**
	 * Adds a list of types to this typed token.
	 * 
	 * @param types the types to add.
	 * @throws NullPointerException if the specified types is null.
	 */
	public void addAllTypes(final List<Symbol> types) throws NullPointerException {
		if (types == null)
			throw new NullPointerException();
	...
	/**
	 * Prints the content of a TypedSymbol list
	 * 
	 * @param symbolsList
	 * @param type
	 * @param i
	 */
	public void printTypedSymbol(List<TypedSymbol> symbolsList, String type) {
		System.out.print(type);
		for (int j = 0; j < symbolsList.size(); j++)
			System.out.print("	Kind: " + symbolsList.get(j).getKind() + "	Image: " + symbolsList.get(j).getImage() + "	Type: "
					+ symbolsList.get(j).getTypes() + "\n	");
	}
}
