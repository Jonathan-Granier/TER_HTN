 ...
 * <p>
 * This implementation is based on the implementation of the STAN planner proposed D. Long and M. Fox. To have more information about this implementation see D.
 * Long, M. Fox (1999). Efficient Implementation of the Plan Graph in STAN. Journal of Artificial Intelligence Research, 10(1):87-115.
 * 
...
extends AbstractHeuristic {
	 ...
	 */
	private final List<BitMatrix> achievers;
	
	/**
	 * The number of propositions of the problem.
	 */
	private final int nb_propositions;
	...
	protected GraphHeuristic(final CodedProblem problem) {
		...
		neg_offset = super.getRevelantFacts().size();
		
		// The number of propositions of the problem
		nb_propositions = neg_offset * 2;
		...
		if (debug) {
			...
			propositions = new String[nb_propositions];
			for (int i = 0; i < neg_offset; i++) {
				final IntExp prop = super.getRevelantFacts().get(i);
				propositions[i] = problem.toString(prop);
				propositions[i + neg_offset] = "(not " + propositions[i] + ")";
			}
		}
		
		// Compute the number of unconditional operators of the problem
		nb_operators = nb_propositions;
		final Map<Signature, List<BitOp>> operators = problem.getOperators();
		for (final Signature sign : operators.keySet()) {
			final List<BitOp> ops = operators.get(sign);
			for (final BitOp op : ops) {
				nb_operators += op.getCondEffects().size();
			}
		}
		...
		if (debug) {
			this.operators = new String[nb_operators];
		}
		
		// Initialize the two arrays that wil contain the preconditions and the effects of the
		// unconditional operators of the planning problem
		preconditions = new BitVector[nb_operators];
		effects = new BitVector[nb_operators];
	...
	protected final int expandPlanningGraph(final BitState state) {
		...
		Arrays.fill(propositions_level, Integer.MAX_VALUE);
		
		// Initialize the initial props_layer level of the planning graph
		props_layer = new BitVector();
		props_layer.or(state);
		
		// Initialize the first proposition level with the specified state
		props_layer.flip(neg_offset, nb_propositions);
		for (int p = state.nextSetBit(0); p >= 0; p = state.nextSetBit(p + 1)) {
			props_layer.clear(p + neg_offset);
		}
		for (int p = props_layer.nextSetBit(0); p >= 0; p = props_layer.nextSetBit(p + 1)) {
			propositions_level[p] = 0;
		}
		
		// Initialize the initial actions level of the planning graph
		ops_layer = new BitVector();
		...
		propositions_mutex.add(new BitMatrix(nb_propositions));
		...
		while (!goal_reached && !level_off) {
			...
			ops_layer.or(props_layer);
			// try only the operator not already in the planning graph
			for (int op = ops_layer.nextClearBit(nb_propositions); op > 0 && op < nb_operators; op = ops_layer.nextClearBit(op + 1)) {
				if (props_layer.include(preconditions[op]) && isMutexFree(preconditions[op], k)) {
					ops_layer.set(op);
			...
			props_layer.or(new_propositions);
			// Update the level of the new propositions
			for (int p = new_propositions.nextSetBit(0); p >= 0; p = new_propositions.nextSetBit(p + 1)) {
				if (propositions_level[p] == Integer.MAX_VALUE) {
					propositions_level[p] = k;
				}
			}
			// Update the operators mutexes at level k
			updateOperatorsMutex(k);
			// Update the propositions mutexes at level k + 1
			updatePropositionsMutex(k + 1);
			// Increment the level of the planning graph
			k++;
			// Check if the goal is reached
			goal_reached = (props_layer.include(bvgoal) && isMutexFree(bvgoal, k));
		}
		return k;
	}
	...
	private void updatePropositionsMutex(final int k) {
		...
		final BitMatrix pmk_1 = propositions_mutex.get(k - 1);
		propositions_mutex.add(pm_k);
		for (int pi = props_layer.nextSetBit(0); pi >= 0; pi = props_layer.nextSetBit(pi + 1)) {
			for (int pj = props_layer.nextSetBit(0); pj >= 0; pj = props_layer.nextSetBit(pj + 1)) {
				if (pi > pj && arePropositionsMutex(pi, pj, k)) {
					pm_k.set(pi, pj);
					pm_k.set(pj, pi);
				}
			}
		}
		level_off = pm_k.equals(pmk_1);
	}
	...
	private void updateOperatorsMutex(final int k) {
		if (k > 1) {
			...
			final BitMatrix omk = new BitMatrix(nb_operators);
			operators_mutex.add(omk);
			final BitMatrix omk_1 = operators_mutex.get(k - 1);
			for (int oi = ops_layer.nextSetBit(0); oi >= 0; oi = ops_layer.nextSetBit(oi + 1)) {
				for (int oj = ops_layer.nextSetBit(0); oj >= 0; oj = ops_layer.nextSetBit(oj + 1)) {
					if (oi > oj && omk_1.get(oi, oj) && areOperatorsMutex(oi, oj, k)) {
						omk.set(oi, oj);
						omk.set(oj, oi);
					} else {
						level_off = false;
			...
			for (int oi = new_operators.nextSetBit(0); oi >= 0; oi = new_operators.nextSetBit(oi + 1)) {
				for (int oj = ops_layer.nextSetBit(0); oj >= 0; oj = ops_layer.nextSetBit(oj + 1)) {
					if (oi > oj && areOperatorsMutex(oi, oj, k)) {
						omk.set(oi, oj);
						omk.set(oj, oi);
						level_off = false;
					}
	...
	protected final int getSumValue() {
		int value = 0;
		for (int p = bvgoal.nextSetBit(0); p >= 0; p = bvgoal.nextSetBit(p + 1)) {
			value += propositions_level[p];
		}
		return value;
	}
	
	/**
	 * Returns <code>true</code> if two operator are mutex at a specified level. Two ops_layer are mutex if the ops_layer are dependent or an operator has a
	 * mutex precondition at level k;
	 * 
	 * @param oi the first fact.
	 * @param oj the second fact.
	 * @param k the level where the test must be done.
	 * @return <code>true</code> if two operator are mutex at a specified level; <code>false</code> otherwise.
	 */
	private boolean areOperatorsMutex(final int oi, final int oj, final int k) {
		if (k == 0) {
			return operators_dependences.get(oi, oj);
		}
		if (!operators_dependences.get(oi, oj)) {
			boolean mutex = false;
			final BitVector ppi = preconditions[oi];
			final BitVector ppj = preconditions[oj];
			final BitMatrix mk = propositions_mutex.get(k);
	...
	/**
	 * Returns <code>true</code> if two facts are mutex at a specified level. Two facts are mutex if at least one operator that produce the facts are mutex in
	 * the previous level k - 1.
	 * 
	 * @param pi the first fact.
	 * @param pj the second fact.
	 * @param k the level where the test must be done.
	 * @return <code>true</code> if two facts are mutex at a specified level; <code>false</code> otherwise.
	 */
	private boolean arePropositionsMutex(final int pi, final int pj, final int k) {
		...
		if (!mutex && k > 0) {
			final BitMatrix ak = achievers.get(k - 1);
			...
			final BitMatrix mak = operators_mutex.get(k - 1);
			mutex = true;
	...
	/**
	 * Returns <code>true</code> if a set of facts is mutex free in a specified proposition level of the planning graph.
	 * 
	 * @param state the facts to be tested.
	 * @param k the level of the graph at which the test is done.
	 * @return <code>true</code> if a set of facts is mutex free in a specified proposition level of the planning graph, <code>false</code> otherwise.
	 */
	...
	/**
	 * Returns <code>true</code> if two ops_layer are dependent, i.e, if an operator delete an precondition or an add effect of the other.
	 * 
	...
	private boolean areDependent(final int oi, final int oj) {
		final BitVector eff_oi = effects[oi];
		final BitVector eff_oj = effects[oj];
		final BitVector pre_oi = preconditions[oi];
		final BitVector pre_oj = preconditions[oj];
		return !areConsistantStates(eff_oi, eff_oj) || !areConsistantStates(eff_oi, pre_oj) || !areConsistantStates(eff_oj, pre_oi) || !areConsistantStates(pre_oi, pre_oj);
	}
	 ...
	 */
	private boolean areConsistantStates(final BitVector s1, final BitVector s2) {
	...
	/**
	 * Return <code>true</code> if the goal is reached in the last proposition level of the expanded planning graph.
	 * 
	 * @return <code>true</code> if the goal is reached in the last proposition level of the expanded planning graph; <code>false</code> otherwise.
	 */
