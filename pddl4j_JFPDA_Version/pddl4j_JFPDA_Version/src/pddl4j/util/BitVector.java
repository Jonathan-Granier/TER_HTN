...
public class BitVector extends BitSet {
	...
	/**
	 * Return the indexes of set bits in the bitVector
	 * @return an array list of integers with the indexes of tasks
	 */
	public final List<Integer> getAllSetBits(){
		List<Integer> setBits = new ArrayList<Integer>();
		int index = -1;
		do{
			index++;
			index = this.nextSetBit(index);
			if(index != -1)
				setBits.add(index);
		}while(index != -1);
		return setBits;
